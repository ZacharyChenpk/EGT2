    ...
    try:
        ...
        if A_active and B_active:
            ...
            if preds[0] == preds[1]:
                ...
                act = preds[0][:-1][::-1]
            if act:
                # return actor_1 + ' and ' + actor_2 + conjugate(verb=' '.join(act),tense=PRESENT,number=PL)
                ...
                raise ValueError
        # print(preds)
        if negated and preds[0][0] not in can_negate:
            preds[0] = ['do'] + preds[0]
        if negated and preds[1][0] not in can_negate:
            preds[1] = ['do'] + preds[1]
        try:
        ...
        if A_active and not B_active:
            ...
            # if 'VERB' not in wordtags[act[0]]:
            #     act[0] = conjugate(verb=act[0],tense=PRESENT,number=SG)
            # else:
            #     act = ['is'] + act
            ...
            mml = min_match_length(orig_preds[0], orig_preds[1])
            if pathway:
                # act = [actor_1, 'is', conjugate(verb=orig_preds[0][0],tense=PAST,aspect=PROGRESSIVE,number=SG)] + orig_preds[0][1:] + orig_preds[1][mml:] + [actor_2]
                if negated:
                    be = 'is not'
                else:
                    be = 'is'
                ...
                if negated:
                    if orig_preds[0][0] not in can_negate:
...
root = "../gfiles/"
model = BertNLIModel(model_path=None,gpu=torch.cuda.is_available(),bert_type='deberta',label_num=3,batch_size=1024,reinit_num=-1,freeze_layers=True)

ratio = 0.8
ratio = str(ratio)
checkpoint_file = 'deberta_tars/deberta0.8_12_1e-05_1_best.pth.tar'
checkpoint = torch.load(checkpoint_file)
print('loading from', checkpoint_file)
model.load_state_dict(checkpoint['state_dict'])

engG_dir_addr = "../gfiles/typedEntGrDir_NS_all_AUG_MC/"
write_addr = "../gfiles/typedEntGrDir_bertnli_modified_aug_tuned_r"+ratio+"/"
...
for f in files:
    if f[-4:] != '.txt':
        continue
    if os.path.exists(write_addr + f):
        file_clean.append(file_dirty)
        file_dirty = f

for f in files:
    if f[-4:] != '.txt' or f in file_clean:
        print(f, 'passed!')
        continue
    print(f, 'start')
    stt = time.time()
    ...
    if values.size(0) != 0:
        idxpair_list = list(indices.t())
        predpair_list = [(gr.allPreds[i], gr.allPreds[j]) for i,j in idxpair_list]
        template_list = [pred2template(p, gr.types) for p in gr.allPreds]
        print('template_list build', 'time:', time.time() - stt)
        templatepair_list = [(template_list[i], template_list[j]) for i,j in idxpair_list]
        print('templatepair_list build', 'time:', time.time() - stt)
        if 'deberta' in model.bert_type:
            values = model(templatepair_list)[1][:,2]
        else:
            values = model(templatepair_list)[1][:,1]
        print('model forward', 'time:', time.time() - stt)
    utils.sen2index_memo = {}
    utils.sen2token_memo = {}
    gr.composing(indices,values,size)
    gr.writeGraphToFile(write_addr + f)
