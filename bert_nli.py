...
class BertNLIModel(nn.Module):
    ...
    def __init__(self,model_path=None,gpu=True,bert_type='bert-base',label_num=3,batch_size=8,reinit_num=0,freeze_layers=False):
        ...
        elif 'deberta' in bert_type:
            self.tokenizer = AutoTokenizer.from_pretrained("microsoft/deberta-base-mnli")
            self.bert = AutoModelForSequenceClassification.from_pretrained("microsoft/deberta-base-mnli")
        else:
        ...
        if 'deberta' in bert_type:
            self.nli_head = lambda x:x
        else:
        ...
        if model_path is not None:
            if gpu:
                sdict = torch.load(model_path,map_location=lambda storage, loc: storage.cpu())
    ...
    def forward(self, sent_pair_list, checkpoint=True, bs=None):
        ...
        for batch_idx in tqdm(range(0,len(sent_pair_list),bs), disable=no_prog_bar,desc='evaluate'):
            probs = self.ff(sent_pair_list[batch_idx:batch_idx+bs],checkpoint)[1]
            if all_probs is None:
                all_probs = probs
            else:
                all_probs = torch.cat([all_probs, probs], dim=0)
        print('building batch time:', self.time_building)
        labels = []
        for pp in all_probs:
            ll = np.argmax(pp.cpu())
            if ll==0:
    ...
    def ff(self,sent_pair_list,checkpoint):
        ...
        self.time_building += (time.time()-start_time)
        ...
        if 'deberta' in self.bert_type:
            cls_vecs = self.bert(input_ids=ids_tensor, token_type_ids=types_tensor, attention_mask=masks_tensor)['logits']
            # print(cls_vecs[:5])
        else:
            if checkpoint:
